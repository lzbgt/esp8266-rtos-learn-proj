...
{
    *err = 0;
    ESP_LOGD(TAG, LOG_FMT("uri: %s [%d] "), uri, uri_len);
    for (int i = 0; i < hd->config.max_uri_handlers; i++) {
        if (hd->hd_calls[i]) {
            ESP_LOGD(TAG, LOG_FMT("[%d] = %s"), i, hd->hd_calls[i]->uri);
...
/**
 *  parse uri with query params (k, v)s in mind
 *  bruce.lu 200322
 */

static httpd_uri_t* httpd_find_uri_handler3(httpd_err_resp_t *err,
                                            struct httpd_data *hd,
                                            const char *uri, size_t uri_len,
                                            httpd_method_t method)
{
    *err = 0;
    ESP_LOGD(TAG, LOG_FMT(" %d uri: %s [%d] "), *err, uri, uri_len);
    for (int i = 0; i < hd->config.max_uri_handlers; i++) {
        if (hd->hd_calls[i]) {
            ESP_LOGD(TAG, LOG_FMT("installed [%d] = %s"), i, hd->hd_calls[i]->uri);
            if ((strlen(hd->hd_calls[i]->uri) == uri_len) &&            // First match uri length
                (strncmp(hd->hd_calls[i]->uri, uri, uri_len) == 0))  {  // Then match uri strings
                if (hd->hd_calls[i]->method == method)  {               // Finally match methods
                    return hd->hd_calls[i];
...
esp_err_t httpd_uri(struct httpd_data *hd)
{
    ...
    if (res->field_set &(1 << UF_QUERY)) {
        ESP_LOGD(TAG, LOG_FMT("query: %s [%d]"), req->uri + res->field_data[UF_QUERY].off, res->field_data[UF_QUERY].len);
        req->len_short = res->field_data[UF_QUERY].off - res->field_data[UF_PATH].off - 1;
    }else{
        req->len_short = res->field_data[UF_PATH].len;
    }
    if (res->field_set & (1 << UF_PATH)) {
        uri = httpd_find_uri_handler3(&err, hd,
    ...
    // parse params
    // note: copy first to preserve uri
    req->num_params = 0;
    if(res->field_set & (1 << UF_PATH)){
        char query[res->field_data[UF_QUERY].len + 1];
        memcpy(query, req->uri + res->field_data[UF_QUERY].off, res->field_data[UF_QUERY].len);
        query[res->field_data[UF_QUERY].len] = 0;
        int idx = 0;
        char *eq = NULL;
        char *kv = strtok (query, "&");
        while (kv != NULL)
        {
            if(idx >= ESP_HTTPD_NUM_MAX_PARAMS) {
                // ignore more params
                break;
            }
            eq = strchr(kv, '=');
            if(eq){
                size_t sz = strlen(kv) - (eq - kv) - 1;
                req->params[idx+1] = calloc(1, sz + 1);
                memcpy(req->params[idx+1], eq+1, sz);
                *eq = 0;
                sz = eq - kv + 1;
                req->params[idx] = calloc(1, sz + 1);
                memcpy(req->params[idx], kv, sz);
        
                ESP_LOGD(TAG, "k: %s, v: %s", req->params[idx], req->params[idx+1]);
                idx +=2;
            }
    ...
    // free params
    for(int i = 0; i < req->num_params * 2; i++){
        free(req->params[i]);
    }
